...
mutable struct copula <: AbstractCopula
    ...
    function copula(cdf=missing; density=missing, func=missing, param=missing)
        if (ismissing(cdf) && ismissing(density))
            throw(ArgumentError("Cdf and/or density must be provided"))
        end
        if (ismissing(cdf))
            cdf = calcCdf(density)
        end
...
function sample(C::AbstractCopula, N::Int64=1; plot=false, useInterp=false)
    ...
    if func !== 0
        conditional =
            (C(x .+ e / 2, ygrid, useInterp) - C(x .- e / 2, ygrid, useInterp)) ./ e
    else
    end
    ...
    for i in 1:N
        ...
        if !issorted(thisCond)
            thisCond = sort(thisCond)
        end
...
function calcDensity(
    cdf::Array{<:Real,2}, xRange=range(0, 1; length=n), yRange=range(0, 1; length=n)
)
    xStep = Float64(xRange.step)
...
function Gau(X, Y, corr=0)
    return [
        bivariate_cdf(quantile.(Normal(), x), quantile.(Normal(), y), corr) for x in X,
        y in Y
    ]
end;
...
function Arch1(X, Y, theta=0)
    return [
        1 - ((1 - x)^theta + (1 - y)^theta - (1 - x)^theta * (1 - y)^theta)^theta for
        x in X, y in Y
    ]
end;
...
function rotate90(C::copula)
    ...
    if !ismissing(C_func)
        if !ismissing(C_param)
            function Rotate90(xs, ys, C_param)
                return [y - C_func(1 - x, y, C_param)[1] for x in xs, y in ys]
            end
...
function rotate180(C::copula)
    ...
    if !ismissing(C_func)
        if !ismissing(C_param)
            function Rotate180(xs, ys, C_param)
                return [x + y - 1 + C_func(1 - x, 1 - y, C_param)[1] for x in xs, y in ys]
            end
            u = v = range(0; stop=1, length=n)
...
function rotate270(C::copula)
    ...
    if !ismissing(C_func)
        if !ismissing(C_param)
            function Rotate270(xs, ys, C_param)
                return [x - C_func(x, 1 - y, C_param)[1] for x in xs, y in ys]
            end
            u = v = range(0; stop=1, length=n)
...
mutable struct Joint <: AbstractJoint
    ...
    function Joint(marginal1, marginal2, copula)
        if isa(copula, ArchimedeanCopula)
            return new(marginal1, marginal2, copula, missing, missing, missing, missing)
        end
...
function (obj::Joint)(X, Y)    # Evaluating the joint cdf
    ...
    if isa(obj.copula, ArchimedeanCopula)
        return cdf.(obj.copula, cdf.(obj.marginal1, X), cdf.(obj.marginal2, Y))
    end
...
struct marginal <: AbstractMarginal
    ...
    function marginal(cdf::Array{Float64,1}, range)
        if (length(cdf) !== length(range))
            throw(ArgumentError("Cdf and range must be the same length"))
        end
        ...
        return new(cdf, range, cdfInp, invCdfInp)
    end
end
